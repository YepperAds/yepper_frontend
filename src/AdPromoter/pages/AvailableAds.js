...
const AvailableAds = () => {
  ...
  const fetchWebsites = async () => {
    try {
      const response = await fetch('https://yepper-backend.vercel.app/api/createWebsite', {
  ...
  const fetchCategories = async (websiteId) => {
    try {
      const response = await fetch(`https://yepper-backend.vercel.app/api/ad-categories/${websiteId}`, {
        headers: getAuthHeaders()
      });
      const data = await response.json();
  ...
  const fetchWalletBalance = async () => {
    try {
      const response = await fetch('https://yepper-backend.vercel.app/api/ad-categories/wallet', {
        headers: getAuthHeaders()
      });
      const data = await response.json();
  ...
  const fetchAvailableAds = async () => {
    setLoading(true);
    try {
      const url = new URL('https://yepper-backend.vercel.app/api/web-advertise/available');
  ...
  const handleAssignAd = async (adId, potentialEarning) => {
    ...
    setAssigning(adId);
    try {
      const response = await fetch('https://yepper-backend.vercel.app/api/web-advertise/assign', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({
          adId,
