...
const AdReports = () => {
  ...
  const fetchWalletBalance = async () => {
    try {
      const response = await axios.get('https://yepper-backend-ll50.onrender.com/api/ad-categories/wallet', {
  ...
  const fetchAdReports = async () => {
    try {
      const [pendingResponse, activeResponse] = await Promise.all([
        axios.get('https://yepper-backend-ll50.onrender.com/api/ad-categories/pending-rejections', {
          headers: getAuthHeaders()
        }),
        axios.get('https://yepper-backend-ll50.onrender.com/api/ad-categories/active-ads', {
          headers: getAuthHeaders()
        })
      ]);
  ...
  const handleRejectAd = async () => {
    ...
    try {
      ...
      await axios.post(
        `https://yepper-backend-ll50.onrender.com/api/ad-categories/reject/${selectedAd._id}/${websiteSelection.websiteId}/${websiteSelection.categories[0]}`,
