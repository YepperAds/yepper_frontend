...
const Wallet = () => {
  ...
  const fetchWalletData = async () => {
    try {
      ...
      const response = await axios.get(`https://yepper-backend.vercel.app/api/ad-categories/wallet/${walletType}/balance`, {
        headers: getAuthHeaders()
      });
  ...
  const fetchTransactions = async (page = 1) => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://yepper-backend.vercel.app/api/ad-categories/wallet/${walletType}/transactions?page=${page}&limit=10`, 
        {
          headers: getAuthHeaders()
        }
      );
