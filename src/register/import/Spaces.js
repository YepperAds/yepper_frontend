      ...
      {categoryDetails.length > 0 ? (
        categoryDetails.map((detail) => (
          <div key={detail.category._id} className="category-section">
            ...
            <h2>{detail.category.categoryName}</h2>
                ...
                <div key={space._id} className="space-item">
                  <label>
                    <input
                      ...
                      disabled={space.remainingUserCount <= 0} // Disable if no more space
                    />
                    ...
                    {space.availability === 'Always available' ? (
                      ...
                      <span className="badge badge-warning">From {new Date(space.startDate).toLocaleDateString()} to {new Date(space.endDate).toLocaleDateString()}</span>
                    ) : (
                    ...
                    {space.instructions && <p><strong>Instructions:</strong> {space.instructions}</p>}
                  </label>
                </div>
              ))
            ) : (
