...
function SelectWebsitesForExistingAd() {
  ...
  const fetchAdDetails = async () => {
    try {
      const response = await axios.get(`https://yepper-backend-ll50.onrender.com/api/web-advertise/${adId}`, {
  ...
  const fetchWebsites = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://yepper-backend-ll50.onrender.com/api/createWebsite', {
        headers: getAuthHeaders()
      });
