...
function SelectWebsitesForExistingAd() {
  ...
  const fetchAdDetails = async () => {
    try {
      const response = await axios.get(`https://yepper-backend.vercel.app/api/web-advertise/${adId}`, {
  ...
  const fetchWebsites = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://yepper-backend.vercel.app/api/createWebsite', {
        headers: getAuthHeaders()
      });
