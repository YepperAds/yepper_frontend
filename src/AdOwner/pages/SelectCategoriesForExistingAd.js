...
const SelectCategoriesForExistingAd = () => {
  ...
  const fetchWalletInfo = async () => {
    try {
      const response = await axios.get(
        'https://yepper-backend-ll50.onrender.com/api/web-advertise/payment/wallet-balance',
    ...
    try {
      const promises = selectedWebsites.map(async (websiteId) => {
        ...
        const categoriesResponse = await axios.get(
          `https://yepper-backend-ll50.onrender.com/api/ad-categories/${websiteId}/advertiser`,
          { headers: getAuthHeaders() }
        );
  ...
  const calculatePaymentBreakdown = async () => {
    try {
      ...
      const response = await axios.post(
        'https://yepper-backend-ll50.onrender.com/api/web-advertise/payment/calculate-breakdown',
        { 
          selections,
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://yepper-backend-ll50.onrender.com/api/web-advertise/${adId}/add-selections`,
        {
  ...
  const handlePayAllCategories = async () => {
    try {
      ...
      const response = await axios.post(
        'https://yepper-backend-ll50.onrender.com/api/web-advertise/payment/process-wallet',
        { 
          selections,
          isReassignment: isReassignment || false
        },
